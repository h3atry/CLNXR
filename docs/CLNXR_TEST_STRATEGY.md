...
| Hard link físico | cria hard link NTFS real com um nome fora da raiz, bloqueia o candidato e verifica que os dois nomes permanecem. | Não cobre ACL negada, concorrência de outro processo ou todos os volumes/FS suportados. |
...
| Envelope de regras assinado | assina payload de catálogo com RSA/SHA-256, aceita a chave correspondente e rejeita payload adulterado ou chave diferente; valida o catálogo depois da assinatura. | É verificação local de contrato; não existe ainda chave pública de produção, canal autenticado, rotação, rollback ou atualização automática. |
